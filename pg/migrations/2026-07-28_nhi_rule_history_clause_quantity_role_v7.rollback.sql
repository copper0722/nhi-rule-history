-- Exact rollback for the reader-enrichment v7 quantity role.
...
CREATE TEMP TABLE rollback_v7_runs ON COMMIT DROP AS
SELECT run_id
FROM nhi_rule_history_clause.reader_enrichment_run
WHERE generator_version = 'chapter-00-reader-enrichment/v7';

DELETE FROM nhi_rule_history_clause.agent_history_summary
WHERE enrichment_run_id IN (SELECT run_id FROM rollback_v7_runs);
...
WHERE run_id IN (SELECT run_id FROM rollback_v7_runs);
