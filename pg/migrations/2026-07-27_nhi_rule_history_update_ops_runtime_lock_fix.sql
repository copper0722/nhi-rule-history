-- 2026-07-27 — serialize lease-bound inserts without table UPDATE privilege
...
DECLARE
  ...
  v1_observation_source text := $source$
DECLARE
  lease_owner text;
...
BEGIN
  IF obj_description(
    ...
    RAISE EXCEPTION
      'runtime-lock fix requires the managed operational schema'
  ...
  IF (
    ...
    RAISE EXCEPTION
      'runtime-lock fix refuses chronology trigger drift'
      USING ERRCODE = 'integrity_constraint_violation';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION
      'runtime-lock fix refuses chronology relation owner drift'
      USING ERRCODE = 'integrity_constraint_violation';
  END IF;
  ...
  SELECT
    ...
    prosrc
    INTO STRICT observation_comment, observation_source
  FROM pg_proc
  WHERE oid =
      'nhi_rule_history_update_ops.guard_owned_observation_insert()'
        ::regprocedure;
  SELECT
    ...
    prosrc
    INTO STRICT worker_comment, worker_source
  FROM pg_proc
  WHERE oid =
      'nhi_rule_history_update_ops.guard_worker_attempt_insert()'
        ::regprocedure;
  ...
  base_state :=
    observation_comment = v1_observation_comment
    AND worker_comment = v1_worker_comment
    AND observation_source = v1_observation_source
    AND worker_source = v1_worker_source;
  target_state :=
    observation_comment = v2_observation_comment
    AND worker_comment = v2_worker_comment
    AND observation_source = v2_observation_source
    AND worker_source = v2_worker_source;
  IF NOT base_state AND NOT target_state THEN
    RAISE EXCEPTION
      'unknown exact chronology function state refuses runtime-lock fix'
      USING ERRCODE = 'integrity_constraint_violation';
  END IF;
...
DO $post_guard$
DECLARE
  expected_observation_comment text :=
    'Serializes on the job/lease advisory key, checks lease-owner row consistency and source chronology; owner_key is metadata, not actor authentication. managed=nhi_rule_history_update_ops/runtime-lock-fix-v2';
  expected_worker_comment text :=
    'Serializes on the job/lease advisory key and refuses attempts that would postdate an already-recorded source observation. managed=nhi_rule_history_update_ops/runtime-lock-fix-v2';
  observation_source text;
  worker_source text;
BEGIN
  SELECT prosrc INTO STRICT observation_source
  FROM pg_proc
  WHERE oid =
    'nhi_rule_history_update_ops.guard_owned_observation_insert()'
      ::regprocedure;
  SELECT prosrc INTO STRICT worker_source
  FROM pg_proc
  WHERE oid =
    'nhi_rule_history_update_ops.guard_worker_attempt_insert()'
      ::regprocedure;
  IF obj_description(
       'nhi_rule_history_update_ops.guard_owned_observation_insert()'
         ::regprocedure,
       'pg_proc'
     ) IS DISTINCT FROM expected_observation_comment
     OR obj_description(
       'nhi_rule_history_update_ops.guard_worker_attempt_insert()'
         ::regprocedure,
       'pg_proc'
     ) IS DISTINCT FROM expected_worker_comment
     OR observation_source NOT LIKE
          '%PERFORM pg_advisory_xact_lock(%'
     OR worker_source NOT LIKE
          '%PERFORM pg_advisory_xact_lock(%'
     OR observation_source LIKE '%FOR UPDATE%'
     OR worker_source LIKE '%FOR UPDATE%' THEN
    RAISE EXCEPTION
      'runtime-lock fix did not establish the exact target family'
      USING ERRCODE = 'integrity_constraint_violation';
  END IF;
END;
$post_guard$;
