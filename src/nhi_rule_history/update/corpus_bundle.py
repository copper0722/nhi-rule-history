                        ...
                        + json.dumps(
                            {
                                ...
                                "artifact_sha256": block[
                                    "artifact_sha256"
                                ],
                                "locator": block["locator"],
                                "raw_text_sha256": block[
                                    "raw_text_sha256"
                                ],
                            },
                            ensure_ascii=False,
...
def prepare_corpus_bundle(
    ...
    *,
    ...
    metadata = extract_notice_metadata(
        detail_payload, detail["artifact_sha256"]
    )
    ...
    if target.exists():
        ...
        if (
            existing.get("source_uid") != source_uid
            or existing.get("origin_update_bundle_fingerprint")
            != source_verification["bundle_fingerprint"]
        ):
        ...
        return {
            ...
            "file_count": len(existing.get("files", [])) + 1,
        }
